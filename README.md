...
- 30 个点构成的呼吸爱心轮廓,正弦驱动的缩放动画;运动模式可在「跳动」(exp(sin) 快速脉冲,约 0.67s/周期)与「呼吸」(exp(sin) 慢速起伏,4s/周期,透明度随同升降)之间切换,透明度与垂直位置可调
- 成对漂浮的永生粒子,伴随天数计数器
...
- 标记爱心(tagged hearts):由宿主层按事件 ID 生成,以流星形式飞入画布下部随机落位,随后进入漂浮/跳动状态;飞入途中每次接触大心心轮廓都会触发一次与自身轨迹平行、指向同一落点的流星雨(减速变淡、尾随其后,落点始终清晰);点击命中时通过回调通知宿主层;事件消失时淡出
...
- 设置:配置日历名称、天数计数器起始日期、画布主题、大心心行为(透明度/运动模式/垂直位置)与界面语言(默认跟随系统,可切换 English/中文)
