...
pub const Particle = struct {
    // Hot path — accessed every frame for every alive particle
    pos: Vec2,
    vel: Vec2,
    lifespan: f32,
    size: f32,
    age: f32,
    flags: ParticleFlags,
    // Cold path — occasionally accessed
    acc: Vec2,
    // Shared: birth_sec when alive, next_free when in free list
    _storage: union {
        birth_sec: f32,
        next_free: usize,
    },
...
pub fn collectAlive() void {
    ...
    for (0..particle_pool_len) |i| {
        if (particle_pool[i].flags.alive) {
            alive_indices[alive_count] = i;
            alive_count += 1;
        } else {
            particle_pool[i]._storage = .{ .next_free = free_head };
            free_head = i;
        }
    }
}
...
pub fn allocParticle(pos: Vec2, elapsed: f32, opts: ParticleOpts) *Particle {
    if (free_head < PARTICLE_POOL_SIZE) {
        const idx = free_head;
        free_head = particle_pool[idx]._storage.next_free;
        particle_pool[idx] = Particle.init(pos, elapsed, opts); // overwrites entire slot, union becomes birth_sec
...
test "collectAlive counts correctly" {
    ...
    particle_pool[1].flags.alive = false;
    collectAlive();
